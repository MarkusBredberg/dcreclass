...
def redistribute_excess(train_images, train_labels,
    ...
    for cls in target_classes:
        items = bins[cls]
        items_sorted = sorted(items, key=lambda x: img_hash(x[0])) # Better for RAW: This sorts by image content so that eval/train split is content-diverse
        #items_sorted = sorted(items, key=lambda x: (hashlib.sha1(str(x[1]).encode("utf-8")).hexdigest(), str(x[1]))) # Better for RT50kpc: This sorts by filename
        # Print out the source names for debugging: e.g. [PSZ2G023.17+86.71, PSZ2G031.93+78.71, ...]
        print(f"Class {cls} items sorted by filename hash: {[x[1] for x in items_sorted]}")
